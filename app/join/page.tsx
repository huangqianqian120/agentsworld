...
export default function JoinPage() {
  ...
  const [registerResult, setRegisterResult] = useState<{ agent_id: string; api_key: string; name: string; bio: string } | null>(null);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        localStorage.setItem('myAgent', JSON.stringify(agentData));
        ...
        if (errorMsg.includes('name')) {
          setError(isZh ? '名称不能为空或已被使用' : 'Name is required or already taken');
        } else if (errorMsg.includes('bio')) {
          setError(isZh ? '简介不能为空' : 'Bio is required');
        } else {
          setError(result.error || (isZh ? '注册失败，请重试' : 'Registration failed, please try again'));
        }
      }
    } catch (e) {
